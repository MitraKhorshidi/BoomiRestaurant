...
const OrderPage =({foods})=>{
    ...
    return(
            ...
            <div className="flex flex-col justify-center items-center gap-y-5 mt-10">
                ...
                <div className="flex flex-row flex-wrap justify-center gap-x-8">
                    {foods.filter( (food)=>shoppingCart[food.id]>0 ).map((food)=>(
                        <CartItem key={food.id} food={food} num={shoppingCart[food.id]} price={food.price*shoppingCart[food.id]}/>
                    ))}
                </div>
                
                <div className="flex flex-row  gap-x-4 mt-4">
                    <button onClick={()=>setMode(MODE_ORDER)} className="text-gray-700 py-1.5 px-2.5 text-lg font-medium text-center rounded-md shadow-md">back to order</button>
                    <button onClick={()=>setMode(MODE_CHECKOUT)} className="bg-pink-200 hover:bg-pink-300 py-1.5 px-2.5 text-lg font-medium text-center rounded-md shadow-md">Checkout</button>
                </div>
        ...
        {mode==MODE_CHECKOUT &&<>
            <form onSubmit={onOrderReq} className="flex flex-col justify-center items-center gap-y-5 mt-10">
                <p className="text-2xl text-title font-medium">Resgistering Order</p>
                <div className="flex flex-row gap-x-5">
                    <div className="flex flex-col gap-y-2">
                        <p>phone number :</p>
                        <input ref={userIdRef} type="tel" className="border-1 border-slate-400 rounded-sm focus:border-black focus:ring-0 h-10"/>
                    </div>
                    <div className="flex flex-col gap-y-2">
                        <p>address :</p>
                        <input ref={addressRef} type="text" className="border-1 border-slate-400 rounded-sm focus:border-black focus:ring-0 h-10"/>
                    </div>
                    
                </div>
                <div className="flex flex-row gap-x-5">
                    <button onClick={()=>setMode(MODE_CART)} className="text-gray-700 py-1.5 px-2.5 text-lg font-medium text-center rounded-md shadow-md">back to cart</button>
                    <input type="submit" value="Register" className="bg-pink-200 hover:bg-pink-300 text-lg px-4 py-1 font-medium rounded-md shadow-sm self-center"/>
                </div>
